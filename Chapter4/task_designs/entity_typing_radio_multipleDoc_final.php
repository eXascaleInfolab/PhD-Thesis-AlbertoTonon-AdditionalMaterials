...
<head>
	...
	<?php
		...
		/**
		* For the specify entity creates a table that allows the worker to 
		* specify that only one type in $types is relevant wrt the context.
		*/
		function createTypesRadioButtons( $docID, $entity, $types ) {
			echo "<br /><table>
			<tr> 
				<th style=\"text-align: left\">{$entity['label']}</th> 
			</tr>";
			...
			foreach( $types_to_print as $label => $uris ) {
				...
				<td>
					...
					<input type=\"radio\" name=\"{$docID}_{$entity['uri']}[]\" value=\"{$docID}_{$entity['uri']}_{$i} \">
					{$label}
				</td>
				</tr>";
				$i++;
...
function checkform (form) {
	...
	* the suggested type (or error) as value. */
	...
	for( element = 0; element < form.elements.length; element++ ) {
		// counting, for each entity, the number of selected types
		if( form.elements[element].type == "radio" ) {
			//to get the (docid,uri) of the entity i just have to remove "[]" from the name of the radio
			var len =  form.elements[element].name.length;
			var entityUri = form.elements[element].name.substr(0, len-2);
			
			if( typeof(typesCount[entityUri]) == "undefined" ) typesCount[entityUri] = 0;
			
			if(	form.elements[element].checked ) {
				typesCount[entityUri]++;
				// alert( "relevant type " + form.elements[element].value + " for " + entityUri);
			}//if
		}//if

		if( form.elements[element].type == "text" && form.elements[element].value != "" ) {
			// to get the entity uri i have to take all the string that precedes "_suggestion"
	...
	for( var count in typesCount ) {
		if( typesCount[count] == 0 && entitySuggestedTypes[count] != "error" ) 	{ 
			alert("Please, check to have selected one type per entity or to have signalled an error " +
...
if( isset($_GET['taskID']) ) {
	...
	else if ( startsWith($taskID, 'P_task') ) $dir = 'paragraphs_tasks';
	else if ( startsWith($taskID, 'P3_task') ) $dir = '3paragraphs_tasks';
...
<?php	
	...
	foreach( $entities as $entity ) {
		$types = $entity["types"];
		createTypesRadioButtons( $task['docID'], $entity, $types );
	}//foreach

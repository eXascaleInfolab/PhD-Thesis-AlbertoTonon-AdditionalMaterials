...
if( isset($_GET['docID']) ) {
	$docID = $_GET['docID'];
	$json = file_get_contents("entities/$docID.json", FILE_TEXT);
...
<h3 style="color: red">Instructions</h3>
	<p style="width: 500px">
		For each entity (person/organization/location) select the <b>one</b> type that best describes it.
	</p>
...
<div id="Instructions" style="width: 500px; display: none">
	<p>For example, the type which best describes "Tom Cruise" is "Actor" rather than "Scientology adept" or "Person".</p>
	
	<p>You can also propose a new type that fits better than all the listed ones by using
	the "Suggest a new type / signal not and entity" field. 
	<emph>In every case you have to say "yes" to at least one of the listed types</emph>.</p>
	
	<p style="width: 500px">If you think that one of the proposed entities is not an entity like, for example,
	"in the kitchen", you should signal it by writing "error" in the "Suggest a 
	new type / signal not an entity" field.</p>

	<p style="width: 500px">Thank you for your time.</p>
</div>

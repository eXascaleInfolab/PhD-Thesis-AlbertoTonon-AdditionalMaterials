...
<head>
	<?php
		...
		* specify that more than one type in $types is relevant wrt the context.
		*/
		...
		*/
		function createTypesRadioButtons($entity, $types ) {
			echo "<br /><table>
			<tr> 
			...
			foreach( $types_to_print as $label => $uris ) {
				...
				<td>
					<input type=\"hidden\" name=\"{$entity['uri']}_{$i}_types\" value=\"{$similar_types_string}|\">
					<input type=\"radio\" name=\"{$entity['uri']}[]\" value=\"{$entity['uri']}_{$i} \">
					{$label}
				</td>
				</tr>";
				$i++;
	...
	* the suggested type (or error) as value. */
	...
	for( var count in typesCount ) {
		if( typesCount[count] == 0 && entitySuggestedTypes[count] != "error" ) 	{ 
			alert("Please, check to have selected at least one type per entity or to signal an error " +
...
<?php	
	...
	foreach( $entities as $entity ) {
		$types = $entity["types"];
		createTypesRadioButtons($entity, $types);
	}//foreach
